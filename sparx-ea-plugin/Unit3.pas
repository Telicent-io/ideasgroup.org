...
//    begin
    ...
    checkElem(rep,holder,errors,true, false, false);
 ...
 //                                           else xsd.Add('							  <xs:element name="' + name + '" minOccurs="0" maxOccurs="unbounded">');
    ...
    fcat := getFoundationCategory(rep,holder.element,supers,errors);
...
//    fcat := 'ideas:Thing';
  ...
  begin
    ...
    xsd.Add('									<xs:complexType>');
    xsd.Add('										<xs:complexContent>');
    xsd.Add('											<xs:extension base="ideas:UniqueNamingScheme">');
    //IC-ISM
    if checkbox1.Checked then xsd.Add('												<xs:attributeGroup ref="ism:SecurityAttributesOptionGroup"/>');
    xsd.Add('											</xs:extension>');
    xsd.Add('										</xs:complexContent>');
  ...
  xsd.Add('				  	<!-- defaultNamingScheme refers to the NamingScheme that defaultNames optionally used in each element belong to --> ');
